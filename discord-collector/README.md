...
- **社区** — 顶栏「社区」：会员头衔、动态广场、**实时聊天室**（文字/图片/短视频 + 头像包）、每日签到、打赏专区（需 MySQL；媒体默认存 `uploads/chat/`）
...
| `POST /api/v1/cards` | 开放建卡（`X-Cards-Api-Key`）；调用格式见 [docs/cards-api.md](./docs/cards-api.md) |
...
| `VITE_UI_PAGES` | `show,fetch,oi` | 部署版可见路由（逗号分隔；评估页加 `eval`） |
| `CARD_LEVEL_CHECK_ENABLED` | `1` | 档位进度状态机（入场 / 分批 TP / SL） |
| `CARD_LEVEL_CHECK_MS` | `3600000` | 档位检查间隔（默认 1h） |
