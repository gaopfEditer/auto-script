...
默认 **http://127.0.0.1:3850/**（`COLLECTOR_UI_PORT`，与 stream-collector 的 3840 错开）。
...
| `ws://127.0.0.1:3850/ws` | 实时网络诊断 + WS 帧推送 |
...
| `COLLECTOR_UI_PORT` | `3850` | UI / WS 端口 |
...
后续可在 `discord-message-ingest.js` 扩展更多 Gateway 事件（编辑、删除、反应等）或对接 Telegram 推送。
