...
前端检查群 id、时间范围与回测：discord-collector UI → `/telegram`（现为 **channel_profiles 实时消息**）。

实时页依赖：
1. `pnpm run collect:ui`（:3851）
2. `cd telegram && python listen.py`（把 `channel_profiles.json` 内群的新消息 POST 到 `/api/telegram/live/ingest`）
